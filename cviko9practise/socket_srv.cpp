...
void initSemaphores() {
    ...
    semEmpty = sem_open(SEM_EMPTY, O_CREAT, 0644, BUFFER_SIZE);
...
void producer(char* item) {
    ...
    strcpy(sharedBuffer->data[sharedBuffer->writeIndex], item);
...
void consumer(char* out) {
    ...
    strcpy(out, sharedBuffer->data[sharedBuffer->readIndex]);
