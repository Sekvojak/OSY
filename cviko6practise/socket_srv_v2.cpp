        ...
        if ( l_read_poll[ 0 ].revents & POLLIN )
            {
                ...
                char resolution[32];
                strncpy(resolution, l_buf, sizeof(resolution)-1);
                resolution[sizeof(resolution)-1] = '\0';

                
                char output_filename[64];
                snprintf(output_filename, sizeof(output_filename), "podzim_%s.jpg", resolution);                
                output_filename[strcspn(output_filename, "!")] = '\0'; // odstráni !

                log_msg(LOG_INFO, "Requested resolution: %s", resolution);

                if (access(output_filename, F_OK) == 0)
                    {
                        log_msg(LOG_INFO, "Cached file found: %s", output_filename);
    
                    
                        int fd = open(output_filename, O_RDONLY);
                        if (!fd)
                        {
                            log_msg(LOG_ERROR, "nepodarilo sa otvorit file");
                            exit(1);
                        }
                        char buff[4096];
                        int rlen;
                        while ((rlen = read(fd, buff, sizeof(buff))) > 0)
                        {
                            write(client_socket, buff, rlen);
                        }
                        close(fd);
                        close(client_socket);
                        return;
                ...
                if (child == 0) {
                    ...
                    execlp("convert", "convert", "-resize", resolution, "podzim.jpg", output_filename, NULL);
                    exit(1);
                ...
                log_msg(LOG_INFO, "Convert finished, closing client socket.");
                close(client_socket);
                break;
