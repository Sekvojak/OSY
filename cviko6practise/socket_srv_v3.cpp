        ...
        if ( l_read_poll[ 0 ].revents & POLLIN )
            {
                ...
                log_msg(LOG_INFO, "Received: %s", l_buf);

                char ouput_filename[512];
                snprintf(ouput_filename, sizeof(ouput_filename), "pozdrav_%s", l_buf);

                char command[1024];
                snprintf(command, sizeof(command),
                        "g++ -DNAME=\\\"%s\\\" pozdrav.cpp -o %s", l_buf, ouput_filename);
                
                if (access(ouput_filename, F_OK) == 0)
                {
                    log_msg(LOG_INFO,"FOUND FILE %s\n", ouput_filename);

                    int fd = open(ouput_filename, O_RDONLY);
                    int lread;
                    char buff[512];
                    if (fd > 0)
                    {
                        while ((lread = read(fd, buff, sizeof(buff))) > 0)
                        {
                            write(client_socket, buff, lread);
                        }
                        close(fd);
                        close(client_socket);
                        return;
