...
{   
    ...
    // ➡️ Program očakáva dva povinné argumenty:
        // adresu servera (ip_or_name),
        // číslo portu (port_number).
    int l_port = 0;
    char *l_host = nullptr;

    if (t_narg < 4)
    {
        log_msg(LOG_ERROR, "Malo argumentov");
        exit(1);
    }
    ...
    // ➡️ Ak nezada IP alebo port → vypíše chybu a skončí.
    if ( !l_host || !l_port )
    {
    ...
    bzero( &l_ai_req, sizeof( l_ai_req ) ); // vyplni strukturu aby neboli nahodne hodnoty, mi vypiseme len tie co ma zaujimaju
    l_ai_req.ai_family = AF_INET; // AF_INET → IPv4 adresu,
    l_ai_req.ai_socktype = SOCK_STREAM; // SOCK_STREAM → TCP socket.
    ...
    sockaddr_in l_cl_addr =  *( sockaddr_in * ) l_ai_ans->ai_addr; // Zoberie adresu zo štruktúry addrinfo, ktorú nám getaddrinfo() vrátilo.
    ...
    char resolution[64];
    snprintf(resolution, sizeof(resolution), "%s!", t_args[3]); // 3 argument
    log_msg(LOG_INFO, "Requested resize: '%s'", resolution);
    write(l_sock_server, resolution, strlen(resolution));
    
    
    // image.img ulozenie 
    int fd = open("image.img", O_WRONLY );
    if (!fd)
    {
        log_msg(LOG_ERROR, "Problem s otvorenim suboru!");
        close(l_sock_server);
        exit(1);
    }
    ...
    char buffer[4086];
    int len;
    while ((len = read(l_sock_server, buffer, sizeof(buffer))) > 0)
    {
        write(fd, buffer, len);
    }

    log_msg(LOG_INFO, "Subor ulozeny ako umage.img");
    close(fd);
    execlp("display", "display", "image.img", NULL);
