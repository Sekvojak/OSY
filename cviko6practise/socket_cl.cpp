...
{   
    ...
    while ( 1 )
    {
        ...
        int ret = poll( l_read_poll, 2, 5000 );
        if (ret < 0)
        {
        ...
        } else if (ret == 0)
        {
            ...
            char op = (rand() % 2 == 0) ? '+' : '-'; 
            char example[64];
            snprintf(example, sizeof(example), "%d%c%d", a, op, b);
            log_msg(LOG_INFO, "Bored... sending random example: %s", example);
            write(l_sock_server, example, strlen(example));
