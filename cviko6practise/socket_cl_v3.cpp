...
{   
    ...
    write(l_sock_server, name, strlen(name));


    int fd = open("runme", O_WRONLY | O_CREAT );
    int lread;
    char result[1024];
    int total_bytes = 0;
    while ((lread = read(l_sock_server, result, sizeof(result))) > 0)
    {   
        write(fd, result, lread);
        total_bytes += lread;
    }
    close(fd);
    ...
    if (total_bytes == 0)
    {
        log_msg(LOG_INFO, "Nic nemam, nic nespoustim");
        close(l_sock_server);
        exit(0);
    }
