...
void* readFromKeyboard(void* arg) {
    int socketToSend = *(int*)arg;
    char buffer[256];
    int lread;
    while ((lread = read(STDIN_FILENO, buffer, sizeof(buffer))) > 0)
    {   
        buffer[lread] = '\0';
        if (!strcasecmp(buffer, STR_CLOSE))
        {
            write(socketToSend, "close\n", 6);
            printf("Closing connection...\n");
            close(socketToSend);
            exit(0);
        }
        write(socketToSend, buffer, strlen(buffer));
        write(socketToSend, "\n", 1);
    }
    return NULL; 
}
...
void* readFromSocket(void* arg) {
    int socketToRead = *(int*)arg;
    char buffer[256];
    int lread;
    while ((lread = read(socketToRead, buffer, sizeof(buffer))) > 0)
    {   
        buffer[lread] = '\0';
        printf("%s", buffer);
        fflush(stdout);
...
int main( int t_narg, char **t_args )
{
    ...
    pthread_create(&t_recv, NULL, readFromSocket, &l_sock_server);
    pthread_create(&t_send, NULL, readFromKeyboard, &l_sock_server);
